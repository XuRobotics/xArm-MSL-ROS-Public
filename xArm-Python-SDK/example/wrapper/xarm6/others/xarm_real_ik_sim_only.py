...
xarm = xarm_sim.XArm6Sim(p, [0, 0, 0])  # Position the robot at [0, 0, 0]
speed = math.radians(150)  # Joint speed in radians per second

# Simulation loop
while p.isConnected():
    xarm.step()  # Update the xArm simulation
    p.stepSimulation()  # Step the PyBullet simulation
    time.sleep(timeStep)  # Maintain real-time synchronization

# Disconnect PyBullet when simulation ends
p.disconnect()
