...
class XArmController:
    ...
    def solve_inverse_kinematics(self):
        ...
        # Extract target position and orientation from the target pose
        target_position = self.target_pose  # [x, y, z]
        target_orientation = self.target_orientation  # [qx, qy, qz, qw]

        # Convert the target orientation (quaternion) to a rotation matrix (if needed)
        # rotation_matrix = self.quaternion_to_matrix(target_orientation)  # Implement or use helper
        rotation_matrix = np.eye(3)  # Placeholder for the rotation matrix
        print(f"Rotation matrix set as identity for now")
        target_pose_matrix = np.hstack([rotation_matrix, np.array(target_position).reshape(3, 1)])  # Combine R and T

        # Flatten the target pose for IK solver
        ee_pose_flat = target_pose_matrix.reshape(-1).tolist()

        # Inverse Kinematics to get joint configurations
        joint_configs = self.xarm_kin.inverse(ee_pose_flat)  # Use IK solver to compute joint angles

        # Reshape and handle multiple solutions (if applicable)
        ...
        # Choose one solution (e.g., the first one)
        joint_angles = joint_configs[0]  # Or implement a selection strategy
        self.jointPoses = joint_angles.tolist()  # Update joint positions for the simulator
