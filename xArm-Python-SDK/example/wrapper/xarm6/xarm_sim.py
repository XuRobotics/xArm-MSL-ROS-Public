  ...
  def step(self):
    ...
    if useIKFast:
      ...
      for joint_config in joint_configs:
        print(joint_config)
      self.jointPoses = [0]*self.n_joints
      ...
      if useNullSpace:
        ...
        self.jointPoses = self.bullet_client.calculateInverseKinematics(self.xarm,xarmEndEffectorIndex, pos, orn, maxNumIterations=50)
    #print("jointPoses=",jointPoses)
    if useDynamics:
      for i in range(xarmNumDofs):
          print(self.jointPoses[i])
          self.bullet_client.setJointMotorControl2(self.xarm, i+1, self.bullet_client.POSITION_CONTROL, self.jointPoses[i],force=5 * 240.)
    else:
      for i in range(xarmNumDofs):
