...
first_episode = next(iter(data.values()))
print(f"Keys in first episode: {list(first_episode.keys())}")
print(f"Shape of training_actions within first episode: {first_episode['training_actions'].shape}")
print(f"Shape of rollout_actions withint second episode: {first_episode['rollout_actions'].shape if 'rollout_actions' in first_episode else 'N/A'}")
...
# skip interval
skip_interval = 4  # pick 1 out of skip_interval episodes
counter = 0
for ep_key in sorted(rollout_data.keys()):
    if counter % skip_interval != 0:
        ...
        print(f"Processing episode: {ep_key}")
        counter += 1
...
for idx, (gt, pred, ax) in enumerate(zip(main_trajs, target_trajs, axes)):
    print(f"Plotting episode {idx + 1}/{len(target_trajs)}")
